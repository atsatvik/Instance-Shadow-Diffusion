    ...
    for i in tqdm(range(len(eval_dataset)), leave=True):
        ...
        with torch.no_grad():
            ...
            pred_gt = eval_gaussian_diffusion.ddim_sample_loop(
                ema_model, lq.shape, model_kwargs={"latent": latent}, progress=True
            )
            ...
            intrinsic_feature = 255.0 * (
                intrinsic_feature.repeat((1, 3, 1, 1)) / 2 + 0.5
            )
            ...
            cv2.imwrite(
                f"experiments/exp_{i:06d}.png",
                tensor2img(
                    torch.cat((gt, intrinsic_feature, pred_gt), dim=-1),
                    min_max=(0, 255.0),
                ),
            )
