...
def main():
    ...
    print(
        f"=> Loaded dataset with number of {len(dataset)} for training and {len(eval_dataset)} for testing"
    )
    ...
    with tqdm(total=TOTAL_ITERS, disable=not accelerator.is_local_main_process) as pbar:
        for total_iter in range(TOTAL_ITERS):
            ...
            loss = gaussian_diffusion.training_losses(
                model, gt, diffusion_t, model_kwargs={"latent": latent}
            )
                ...
                if (total_iter + 1) % (ACCUM * 500) == 0:
                    if accelerator.is_main_process:
                        ...
                        for i in tqdm(range(len(eval_dataset)), leave=False):
                            ...
                            mask = (
                                batch["mask"].unsqueeze(0).to(device, non_blocking=True)
                            )
                            mask = dilation(mask, torch.ones(21, 21).to(mask.device))
                            ...
                            pred_gt = 255.0 * (pred_gt / 2 + 0.5)
                            lq = 255.0 * (lq / 2 + 0.5)
                            mask = 255.0 * (mask / 2 + 0.5)
                            gt = 255.0 * (gt / 2 + 0.5)
                            psnr = 20 * torch.log10(
                                255.0 / torch.sqrt(torch.mean((pred_gt - gt) ** 2))
                            )
                            avg_psnr.append(psnr)

                        accelerator.log(
                            {"testing_psnr": sum(avg_psnr) / len(avg_psnr)},
                            step=total_iter,
                        )
                        cv2.imwrite(
                            f"experiments/exp_{total_iter:06d}.png",
                            tensor2img(
                                torch.cat((lq, mask, pred_gt, gt), dim=-1),
                                min_max=(0, 255.0),
                            ),
                        )
...
class ISTDDataset(data.Dataset):
    def __init__(self, opt):
        ...
        self.gt_folder, self.lq_folder = join(opt["dataroot"], "shadow_free"), join(
            opt["dataroot"], "shadow"
        )
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            # flip, rotation
            img_gt, img_lq, img_mask = augment(
                [img_gt, img_lq, img_mask], self.opt["use_hflip"], self.opt["use_rot"]
            )

        # BGR to RGB, HWC to CHW, numpy to tensor
        img_gt, img_lq, img_mask = img2tensor(
            [img_gt, img_lq, img_mask], bgr2rgb=True, float32=True
        )
